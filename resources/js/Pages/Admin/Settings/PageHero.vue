...
const heroes = usePage().props.heroes;

const addHero = () => {
    router.get(route('pagehero.create'));
}
const editHero = (hero) => {
    router.get(route('pagehero.edit', hero.id));
}
...
const deleteHero = async (hero) => {
    ...
    if (result.isConfirmed) {
        try {
            await router.delete(route('pagehero.destroy', hero.id));
            router.visit(route('pagehero'));
            Swal.fire({
                toast: true,
                icon: 'success',
                position: 'top-end',
                ...
                timer: 3000,
                title: 'hero deleted successfully'
            })
        } catch (error) {
            Swal.fire({
                toast: true,
                        ...
                        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                            <div
                                ...
                                <div
                                    ...
                                    <Link type="button" @click="addHero"
                                ...
                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    <thead
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col" class="px-4 py-3">sub-caption</th>
                                            <th scope="col" class="px-4 py-3">Contact Title</th>
                                            <th scope="col" class="px-4 py-3">Contact Link</th>
                                            <th scope="col" class="px-4 py-3">Learn More</th>
                                            <th scope="col" class="px-4 py-3">Learn More Link</th>
                                    ...
                                    <tbody>
                                        <tr v-for="hero in heroes" :key="hero.id"
                                            ...
                                            <td class="px-4 py-3">{{ hero.caption }}</td>
                                            <td class="px-4 py-3">{{ hero.sub_caption }}</td>
                                            <td class="px-4 py-3">{{ hero.contact_title }}</td>
                                            <td class="px-4 py-3">{{ hero.contact_link }}</td>
                                            <td class="px-4 py-3">{{ hero.learn_title }}</td>
                                            <td class="px-4 py-3">{{ hero.learn_link}}</td>
                                           
                                            <td class="px-4 py-3">
                                                <button type="button" @click="editHero(hero)"
                                            ...
                                            <td class="px-4 py-3">
                                                <button type="button" @click="deleteHero(hero)"
