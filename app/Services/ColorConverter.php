...
class ColorConverter
{
    ...
    protected $cache = [];
    //protected $similarityThreshold = 40; // Adjust as needed


    public function hexToTailwind(string $hex)
    {
        // Check cache first
        if (isset($this->cache[$hex])) {
            return $this->cache[$hex];
        }
    
        // Normalize hex (remove # if present and convert to lowercase)
        $hex = strtolower(ltrim($hex, '#'));
        
        // Check direct mapping first
        if (isset($this->colorMap['#'.$hex])) {
            $this->cache[$hex] = $this->colorMap['#'.$hex];
            return $this->colorMap['#'.$hex];
        }
    
        // If we get here, we need to find the closest match
        $closestColor = null;
        ...
        foreach ($this->tailwindPalette as $colorName => $shades) {
            foreach ($shades as $shade => $shadeHex) {
                $shadeHex = ltrim($shadeHex, '#');
                $r2 = hexdec(substr($shadeHex, 0, 2));
                $g2 = hexdec(substr($shadeHex, 2, 2));
                $b2 = hexdec(substr($shadeHex, 4, 2));
        ...
        // Cache the result
        $this->cache[$hex] = $closestColor;
        return $closestColor;
        
        // Return the closest color if it's close enough
        // if ($minDistance < $this->similarityThreshold) {
        //     return $closestColor;
        // }
    
        // If no close match, return the original hex with a warning
       // Log::warning("No close Tailwind match found for color: #{$hex}");
        return "#{$hex}"; // or return a default like 'gray-500'
    }
