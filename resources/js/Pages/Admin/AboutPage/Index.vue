...
const submit = () => {
    if (isEditing.value) {
        form.post(route('aboutpage.update', currentAboutId.value), {
            _method: 'put',
            onSuccess: () => {
                resetForm();
                closeModal();
                router.visit(route('aboutpage'));
                Swal.fire({
                    toast: true,
                    icon: 'success',
                    position: 'top-end',
...
const addAbout = () => {
    router.get(route('aboutpage.create'));
}

const showAbout = (about) => {
    router.get(route('aboutpage.show', about.id));
}

const editAbout = (about) => {
    router.get(route('aboutpage.edit', about.id));
}


const deleteAbout = async (about) => {
    ...
    if (result.isConfirmed) {
        try {
            await router.delete(route('aboutpage.destroy', about.id));
            router.visit(route('aboutpage'));
            Swal.fire({
                toast: true,
                icon: 'success',
                                ...
                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    <thead
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col" class="px-4 py-3">Section Header</th>
                                            <th scope="col" class="px-4 py-3">Section Mission & Vision</th>
                                            <th scope="col" class="px-4 py-3">Section Slogan</th>                                            
                                            <th scope="col" class="px-4 py-3">Section Team</th>
                                            <th scope="col" class="px-4 py-3">Section Team Members</th>
                                    ...
                                    <tbody>
                                        <tr v-for="about in abouts" :key="about.id"
                                            ...
                                            <td class="px-4 py-3">{{ about.id }}</td>
                                            <td class="px-4 py-3">{{ about.sectn_header === 1 ? 'On' : 'Off' }}</td>
                                            <td class="px-4 py-3">{{ about.sectn_mivi === 1 ? 'On' : 'Off' }}</td>
                                            <td class="px-4 py-3">{{ about.sectn_slogan === 1 ? 'On' : 'Off' }}</td>
                                            <td class="px-4 py-3">{{ about.sectn_team === 1 ? 'On' : 'Off' }}</td>
                                            <td class="px-4 py-3">{{ about.sectn_team_mem === 1 ? 'On' : 'Off' }}</td>
                                            <td class="px-4 py-3">
                                                <button type="button" @click="editAbout(about)"
                                            ...
                                            <td class="px-4 py-3">
                                                <button type="button" @click="showAbout(about)"
                                            ...
                                            <td class="px-4 py-3">
                                                <button type="button" @click="deleteAbout(about)"
                                                    class="block py-2 px-4 font-semibold hover:rounded-md text-red-600 hover:bg-red-100 dark:hover:bg-red-600 dark:hover:text-white">
                                                    Delete</button>
                                            </td>
